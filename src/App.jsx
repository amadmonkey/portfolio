...
const App = () => {
	...
	d.addEventListener('scroll', () => {
		...
		if(window.pageYOffset > (sections.illustrations.getBoundingClientRect().top + (sections.illustrations.clientHeight*4.8))){
		...
		if(window.pageYOffset > (sections.about.getBoundingClientRect().top + (sections.about.clientHeight*1.7))){
	...
	return (
		<div className="app">
			<Nav></Nav>
			{ navList.map(navItem => navItem.isEnabled && <React.Fragment>{ navItem.component }</React.Fragment>) }
			<BackToTop></BackToTop>
			<Footer></Footer>
		</div>
	);
